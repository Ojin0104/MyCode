...
public class Main {
	...
	static void dfs(int row,int col,int status) {
		...
		for(int next=0;next<=2;next++) {
			...
			if(next_r<0||next_r>=N||next_c<0||next_c>=N||map[next_r][next_c]==1)continue;//범위를 벗어나거나, 벽이있는 경우
			
			if(next==2&&(map[row+1][col]==1||map[row][col+1]==1))continue; // 파이프를 움직이다가 벽에 부딪히는 경우
