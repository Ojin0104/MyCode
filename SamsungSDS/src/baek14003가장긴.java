...
public class baek14003가장긴 {
    ...
    public static void main(String[] args) throws NumberFormatException, IOException {
        ...
        for(int i = 2 ; i <= N ; i++) {
            int searchIndex = doBinarySearch(D,Numbers[i],1,Length);//LIS배열에서 i번째 숫자에 해당하는 값을 가지는 D[1]~D[length]사이에있는  index 값
...
Answer=Length;
        ...
        for(int i = N ; i >= 1 ; i--) {//D가 아닌 Tracking 쓰는 이유: D는 length i를 가진 최소 값이여서 length가 4인 부분순열에 포함안되는 값일 수 있어 최대 length를 먼저 구하고 그 앞에있는 값들을 부분순열에 넣어주기위해서이다.
    ...
    public static int doBinarySearch(int[] array, int key, int from, int to) {//D배열에서 key에 해당하는 값의 위치를 찾음 같다면 크다면
        int low = from; // 왼쪽 범위
        int high = to; // 오른쪽 범위
